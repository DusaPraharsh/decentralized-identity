...
contract ERC721Identifier is ERC721Enumerable, AccessControl {
    ...
    function _beforeTokenTransfer(
        ...
        require(
            (balanceOf(to) == 0),
            "Can only hold one digitalID per wallet, this wallet already has one!"
        );
    ...
    function mint(address to) public {
        ...
        console.log(
            "%s has been given a token with id %s and uri %s",
            to,
            newTokenId,
            tokenURI(newTokenId)
        );
        emit NewIdentifierMinted(
            msg.sender,
            to,
            newTokenId,
            tokenURI(newTokenId),
